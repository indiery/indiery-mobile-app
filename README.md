...
Before a store submission, confirm that `Indiery` is the exact developer/operator name used in Google Play Console, that `support@indiery.com` is monitored, and that any legally required business address or named grievance officer is added for the actual operating entity. Complete the Play Console declarations from `docs/PLAY_STORE_RELEASE_CHECKLIST.md`; policy text does not replace an accurate Data safety form or an operating account-deletion process.
